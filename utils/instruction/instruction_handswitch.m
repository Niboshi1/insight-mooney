function instruction_handswitch(window, cfg, tfun)
    ...
    if strcmp(cfg.handNow, 'left')
        ...
        instruct_text = 'Please switch to your right hand for response.';
        img = imresize(img, imgW_pre/size(img,2));
    ...
    elseif strcmp(cfg.handNow, 'right')
        ...
        instruct_text = 'Please switch to your left hand for response.';
        img = imresize(img, imgW_pre/size(img,2));
        imgW = size(img, 2);  imgH = size(img, 1);
